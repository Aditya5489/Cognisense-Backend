import os, json, time, random, hashlib, warnings, tempfile, traceback
...
def _db():
    """Raw connection with RealDictCursor as default cursor factory,
    so rows behave like dicts (row['col']) and dict(row) works everywhere
    the old sqlite3.Row code expected that."""
...
def get_db():
    """
    Context manager that yields a live connection, commits on success,
    rolls back on error, and always closes the connection afterwards.
    Use like:
        with get_db() as db:
...
def update_profile():
    ...
    with get_db() as db:
        with db.cursor() as cur:
            cur.execute(
                "UPDATE users SET full_name=%s, age=%s, phone=%s, caretaker_name=%s, "
                "caretaker_phone=%s, avatar=%s WHERE id=%s",
...
def history():
    ...
    return jsonify({
        "history": [
            {
                "id":             str(r["id"]),
                "date":           r["taken_at"].isoformat() if hasattr(r["taken_at"], "isoformat") else r["taken_at"],
...
def get_assessment(aid):
    ...
    with get_db() as db:
        with db.cursor() as cur:
            cur.execute(
                "SELECT * FROM assessments WHERE id=%s AND user_id=%s", (aid, uid)
            )
            row = cur.fetchone()
    if not row:
        return jsonify({"error": "Not found"}), 404
    r = dict(row)
    if hasattr(r.get("taken_at"), "isoformat"):
...
def get_game_scores():
    ...
    with get_db() as db:
        with db.cursor() as cur:
            query = "SELECT * FROM game_scores WHERE user_id=%s"
            params = [uid]
            if game:
    ...
    return jsonify({
        "scores": [
            {**dict(r), "played_at": r["played_at"].isoformat() if hasattr(r["played_at"], "isoformat") else r["played_at"]}
            for r in rows
        ]
    })
    ...
    with get_db() as db:
        with db.cursor() as cur:
            cur.execute(
                ...
                "WHERE g.game_name=%s ORDER BY g.moves ASC, g.time_secs ASC LIMIT 10",
                (game,)
            )
            rows = cur.fetchall()
    return jsonify({
